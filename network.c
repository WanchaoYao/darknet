...
// 将网络每一层的 weight bias scale 的 updates 全部置为0
void harmless_update_network_gpu(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        // 将 weight bias scale 的 updates 全部置为0
...
// 将 layer base 的 bias 和 weight 从主机内存拷贝到 layer l 的GPU内存
...
void sync_layer(network **nets, int n, int j)
{
    ...
    // 将网络 第 j 层 layer 拷贝到 base，并将 bias 和 weight 初始化为 0
    layer base = net->layers[j];
    scale_weights(base, 0);

    // 将所有网络第 j 层的 bias 和 weight 累加到 base，并求平均
    for (i = 0; i < n; ++i) {
    ...
    // 将 base 的 bias 和 weight 拷贝到所有网络第 j 层的GPU内存
    for (i = 0; i < n; ++i) {
        cuda_set_device(nets[i]->gpu_index);
        layer l = nets[i]->layers[j];
