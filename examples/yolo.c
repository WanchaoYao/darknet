...
void train_yolo(char *cfgfile, char *weightfile)
{
    ...
    // 从完整路径中截取出 文件名(不包含.后缀名)
    char *base = basecfg(cfgfile);
    printf("%s\n", base);

    // 根据cfg文件和weight文件加载网络
...
void validate_yolo(char *cfg, char *weights)
{
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        // 创建线程，读取图像，对应图像id [i, min(m - 1, i + nthread - 1)]
        ...
        // 检测图像中的物体，对应图像id [i - nthreads, i - 1]
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            // 获取图像文件名
...
// 在验证集上计算 bbox objectness 的 recall
void validate_yolo_recall(char *cfg, char *weights)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        // 根据 image path 和替换规则，构建 label path
        char labelpath[4096];
        ...
        // 给每一个 ground truth 匹配 iou 最大的 bbox
...
// YOLO入口，根据入参区分 train/valid/test
