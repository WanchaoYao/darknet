...
typedef struct network{
    int n;              // 网络层数
    int batch;          // batch * subdivisions = 一个 batch 包含的图片数量
    size_t *seen;       // 网络已经看到的图片数量
    int *t;
    float epoch;
    int subdivisions;   // batch * subdivisions = 一个 batch 包含的图片数量
    layer *layers;
    float *output;      // 网络的输出
...
typedef struct load_args{
    ...
    float jitter;       // 利用抖动(jittering)生成额外数据
    float angle;        // 旋转角度
    float aspect;       
